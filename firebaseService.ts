...
export const subscribeToSocialLinks = (callback: (links: SocialLink[]) => void) => {
    const q = collection(db, 'social_links');
    return onSnapshot(q, (snapshot) => {
        const links: SocialLink[] = [];
        snapshot.forEach((doc) => links.push({ id: doc.id, ...doc.data() } as SocialLink));
        callback(links);
    });
};

export const updateSocialLink = async (id: string, data: Partial<SocialLink>) => {
    await setDoc(doc(db, 'social_links', id), data, { merge: true });
};

export const addSocialLink = async (data: Omit<SocialLink, 'id'>) => {
